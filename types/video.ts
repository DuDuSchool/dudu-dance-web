...
export async function fetchVideoIndex(): Promise<VideoIndexResponse> {
  interface DuDuIndexResponse {
    timestamp: number,
    videos: DuDuVideo[],
    groups: DuDuGroup[],
  }
  ...
  let dudu = await fetchWithDefaults("https://api.dudufit.dance/api/v1/videos") as DuDuIndexResponse;
  return {
      code: 200,
      data: {
        time: dudu.timestamp.toString(),
        groups: dudu.groups.map((group) => {
          let entries = dudu.videos.filter((video) => group.videos.includes(video.id)).map((video) => ({
            id: video.id,
            ayaId: null,
            name: video.name,
            artist: video.artist,
            dancer: video.dancer,
            playerCount: 0,
            composedTitle: `${video.name} - ${video.artist} | ${video.dancer}`,
            composedTitleSpell: `${video.name} - ${video.artist} | ${video.dancer}`,
            group: group.name,
            genre: "",
            volume: video.volume,
            start: video.start,
